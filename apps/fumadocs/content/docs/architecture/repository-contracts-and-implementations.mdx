...
3. **Docs**: In `apps/fumadocs/content/docs/architecture/*.mdx`, replace any remaining references to the old interface name (e.g. `IYourEntityRepository`) with the new one (`YourEntityRepositoryContract`).
