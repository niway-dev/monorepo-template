...
1. **Interface (Contract)** → `packages/domain/` - What the repository can do
2. **Implementation** → `packages/database/` - How it actually does it
...
packages/
  ...
  database/
    your-context/
      ...
      mappers/
        your-entity.mapper.ts              # Maps DB rows ↔ Domain entities
    schema/
...
- ✅ Works with **Domain Entities**, not raw DB data
- ✅ Returns **rich domain objects** like `YourEntity`
- ✅ No SQL, no ORM references
...
- ✅ Uses Drizzle for actual DB operations
- ✅ Uses **Mappers** to convert DB rows ↔ Domain entities
...
- ✅ Can add validation, authorization, etc.
...
### ❌ Bad: Queries Folder in Database Package
...
### ❌ Bad: Repository Returning Raw DB Data
