...
export const yourEntityRoutes = new Elysia({ prefix: "/api/your-entities" })
  .put(
    ...
    },
    {
      ...
      body: updateYourEntitySchema, // 👈 Zod schema from domain
    },
  );
```
...
- Review [Domain Architecture Patterns](/domain-architecture-patterns) for architectural decisions
- See [Backend API Response Types](/backend/api-response-types) for API patterns
- Check [Constants Pattern](/constants-pattern) for using constants with schemas
