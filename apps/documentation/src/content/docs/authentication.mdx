...
```
Before:
Browser (localhost:3001) → Backend (localhost:3000)
Result: Cookies blocked (different origins)

After:
Browser → Frontend /api/auth → Proxy → Backend
...
- Poor SEO for authenticated pages

### Our Full-Stack Solution

- Server-side session validation
- Cookie handling via proxy
- Server validates before render
...
| ---------------------- | ------------------ | ----------------------- |
| Server-side validation | middleware.ts      | beforeLoad              |
| Cookie handling        | Built-in           | Custom proxy            |
| Session caching        | React cache        | Multi-layer             |
| Type safety            | Server Actions     | Server Functions        |
| Performance            | Fast               | Faster (better caching) |
...
- **Encrypted cookies** - Better Auth handles encryption

### Security Flow

```
User → Browser (no session access)
         ↓
...
```
User loads page → getSession() → Database query
...
```
User loads page → Check cache → Return cached
...
If you don't see "session →" logs on every navigation, caching is working!
...
- [Complete Architecture Overview](/authentication/overview) - Deep dive into the full implementation
- [Quick Reference Guide](/authentication/quick-reference) - Common patterns and code snippets
- [Implementation Details](/authentication/implementation) - How we built this full-stack system
